...
class TestDeleteEmailsBySender:
    ...
    def test_valid_email_format(self):
        ...
        with patch("app.services.gmail.delete.get_gmail_service") as mock_get:
            mock_service = Mock()
            mock_get.return_value = (mock_service, None)
    ...
    def test_valid_domain_format(self):
        """Valid domain format should be accepted."""
        with patch("app.services.gmail.delete.get_gmail_service") as mock_get:
            mock_service = Mock()
            mock_get.return_value = (mock_service, None)
            mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = {
    ...
    @patch("app.services.gmail.delete.get_gmail_service")
    def test_no_emails_found(self, mock_get_service):
        """No emails found should return success with zero deleted."""
        mock_service = Mock()
        mock_get_service.return_value = (mock_service, None)
        mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = {
            "messages": []
    ...
    def test_successful_delete(self, mock_get_service):
        """Successful delete should return correct count."""
        mock_service = Mock()
        mock_get_service.return_value = (mock_service, None)

        # Mock list to return messages
        mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = {
    ...
    @patch("app.services.gmail.delete.get_gmail_service")
    def test_delete_with_pagination(self, mock_get_service):
        """Delete should handle pagination correctly."""
        mock_service = Mock()
        mock_get_service.return_value = (mock_service, None)
        ...
        def list_side_effect():
            call_count[0] += 1
            if call_count[0] == 1:
                return {"messages": [{"id": f"msg{i}"} for i in range(500)], "nextPageToken": "token1"}
            else:
                return {"messages": [{"id": f"msg{i}"} for i in range(500, 600)]}

        mock_list.execute.side_effect = list_side_effect
        mock_service.users.return_value.messages.return_value.list.return_value = mock_list
    ...
    def test_delete_updates_cached_results(self, mock_get_service):
        ...
        state.delete_scan_results = [
            {"email": "keep@example.com", "count": 5},
            {"email": "delete@example.com", "count": 10},
        ]
...
class TestDeleteEmailsBulkBackground:
    ...
    @patch("app.services.gmail.delete.get_gmail_service")
    def test_no_emails_found(self, mock_get_service):
        """No emails found should complete without error."""
        mock_service = Mock()
        mock_get_service.return_value = (mock_service, None)
        mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = {
            "messages": []
    ...
    def test_successful_bulk_delete(self, mock_get_service):
        """Successful bulk delete should update status correctly."""
        mock_service = Mock()
        mock_get_service.return_value = (mock_service, None)
